...
// 1. PIN Model (Auto-deletes after 24 hours)
const fileSchema = new mongoose.Schema({
  pin: String,
  whatsapp_id: String,
  filename: String,       // Stores: 1234.pdf
  original_name: String,  // Stores: math_homework.pdf
  mime_type: String,
...
const fileHistorySchema = new mongoose.Schema({
  url: String,       
  pathname: String,  // Stores: 1234.pdf
  ...
  try {
    ...
    const html = `
      ...
      <head>
        ...
        <title>OneTapKiosk Server</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <meta http-equiv="refresh" content="5"> 
      </head>
      <body class="bg-slate-100 min-h-screen p-6 md:p-12 font-sans">
        <div class="max-w-5xl mx-auto">
          <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 border border-slate-200">
            <div class="bg-[#0f172a] p-8 flex flex-col md:flex-row justify-between items-center">
              <div>
                <h1 class="text-3xl font-bold text-white mb-2">OneTapKiosk</h1>
                <p class="text-slate-400">Live Document Monitor</p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
            <div class="overflow-x-auto">
              <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th class="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">PIN</th>
                    <th class="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Filename</th>
                ...
                <tbody class="divide-y divide-slate-100">
                  ${files.length > 0 ? rows : `<tr><td colspan="4" class="p-8 text-center text-slate-400">No files received yet.</td></tr>`}
                </tbody>
              </table>
            </div>
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-200 flex justify-end">
               <form action="/api/clear-all" method="POST" onsubmit="return confirm('Delete all files?');">
                 <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-semibold px-4 py-2">
                   Clear History
                 </button>
               </form>
            </div>
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Handle Text
    if (msgType === "text") {
      await sendMessage(businessPhoneId, from, "👋 Welcome to OneTapKiosk!\nSend a document to print.");
    } 
    // Handle Document/Image
      ...
      const pinFilename = `${pin}${ext}`; // e.g. "1234.pdf"
...
app.get("/download/:pin", async (req, res) => {
  try {
    ...
    // Serve with the PIN filename (e.g., 1234.pdf)
...
app.get('/api/files', async (req, res) => {
  try {
    ...
    const response = files.map(file => ({
      url: file.url,
      pathname: file.pathname, // "1234.pdf"
      date: file.createdAt
    }));
